...
static void  httpdProcessRequest(HttpdConnData *conn) {
	...
	while (1) {
		...
		httpd_printf("About to call conn->cgi");
		r=conn->cgi(conn);
		httpd_printf("About to check if cgi more");
		if (r==HTTPD_CGI_MORE) {
			httpd_printf("There's more data");
